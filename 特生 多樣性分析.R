...
# summary
DataInfo(incidence_dataset[["1010"]]["8 哺乳類"], datatype = "incidence_raw")

estimateD(incidence_dataset[["1010"]]["8 哺乳類"], datatype = "incidence_raw")

ChaoRichness(incidence_dataset[["1010"]]["8 哺乳類"], datatype = "incidence_raw")

#稀釋曲線
l <- iNEXT(row_sum[["1010"]][["8 哺乳類"]], q = 0,datatype = "incidence_freq")
l <- iNEXT(incidence_dataset[["1010"]]["8 哺乳類"], q = 0,datatype = "incidence_raw")
ggiNEXT(l, type = 1)
ggiNEXT(l, type = 2)
ggiNEXT(l, type = 3)
...
# 在固定coverage下
suppressWarnings(
estimateD(incidence_dataset[["1010"]]["8 哺乳類"],
          datatype = "incidence_raw", base = "coverage", level = 0.9)
)

suppressWarnings(
  estimateD(incidence_dataset[["1010"]][["8 哺乳類"]],q = 0,
            datatype = "incidence_raw", base = "coverage", level = 0.9)
)


######max.chao2
max.chao2 <- function(data,level){
  t <- ncol(data)
  suggestion_t <- suppressWarnings(
    estimateD(data,q = 0,
              datatype = "incidence_raw", base = "coverage", level = level)
  )
  suggestion_t <- ceiling(suggestion_t$t)
  
  t <- min(t,suggestion_t)
  
  coln <- colnames(data)
  all_combinations <- combn(coln, t)
  chao <- list()
  ...
  list <- c()
  for (i in 1:5) {
    list[i] <- list(top5_chao_combinations[[i]])
  }
  return(list)
}

##example
out <- max.chao2(incidence_dataset[["1010"]][["8 哺乳類"]], 0.9)
